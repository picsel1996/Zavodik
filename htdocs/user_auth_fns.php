...
$IPb = $_SERVER["REMOTE_ADDR"]=="127.0.0.1";
...
// return true or error message
{
 // connect to db
  ...
  $result = mysqli_query("select * from v_personal where login='$username'"); 
  if (!$result)
  ...
  // if ok, put in db
  $result = mysqli_query("insert into personal (login, passwd, Name, id_TypePers ) values 
...
// else return false
{
  ...
  // check if username is unique
    ...
    $Crypt_pass = crypt($password,'2r');    
    $query = "select * from v_users where login ='$username' and password = '$Crypt_pass'";    
    $result = mysqli_query($conn, $query);
        
    //echo "<br> mysqli_num_rows= ", mysqli_num_rows($result);
    $query = "insert into logs (DT,login,state,ip) values ('".date("Y-m-d H:i:s")."','$username',".(mysqli_num_rows($result)>0?1:0).",'".$_SERVER['SERVER_ADDR']."')";
    //echo "<br> query =", $query;
...
{
  	$res = mysqli_query("SELECT * FROM v_users where login='$username'") or die(mysqli_error());
	$row_Pers = mysqli_fetch_assoc($res);
	return $row_Pers;
}
function f_get_TypePers($username)
{
	$q_TabNum = "SELECT * FROM users where login='$username'";
  	$TabNum = mysqli_query($q_TabNum) or die(mysqli_error());

	$row_TabNum = mysqli_fetch_assoc($TabNum);
	return $row_TabNum["id_type"];
}

function f_get_TabNum($username)//TabNum = id_user
{
	$q_TabNum = "SELECT * FROM users where login='$username'";
  	$TabNum = mysqli_query($q_TabNum) or die(mysqli_error());

	$row_TabNum = mysqli_fetch_assoc($TabNum);
	return $row_TabNum["id_user"];
}

function check_valid_user()
// see if somebody is logged in and notify them if not
{
  if (isset( $GLOBALS["valid_user"]))
  {
